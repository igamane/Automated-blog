...
async function createArticleOnShopify(title, htmlContent, metaDescription) {
  // Article data
  const articleData = {
    article: {
      blog_id: BLOG_ID,
  ...
  if (rewrittenContent) {
    ...
    if (rewrittenTitle) {
      ...
      if (metaDescription) {
        ...
        createArticleOnShopify(rewrittenTitle, rewrittenContent, metaDescription);
      } else {
